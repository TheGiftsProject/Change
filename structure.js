...
World.prototype.generatePatternFor = function(coord) {
    ...
    if (!top_exists || top_connected) {
        top_connected = top_connected || Math.roll(connection_chances[new_connections], World.VARIANCE_CHANCE);
        new_connections++;
    }

    if (!right_exists || right_connected) {
        right_connected = right_connected || Math.roll(connection_chances[new_connections], World.VARIANCE_CHANCE);
        new_connections++;
    }

    if (!bottom_exists | bottom_connected) {
        bottom_connected = bottom_connected || Math.roll(connection_chances[new_connections], World.VARIANCE_CHANCE);
        new_connections++;
    }

    if (!left_exists || left_connected) {
        left_connected = left_connected || Math.roll(connection_chances[new_connections], World.VARIANCE_CHANCE);
    }	
